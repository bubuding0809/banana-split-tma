...
- **Read-only summary Cell at top** — same shape as the row the user just
  tapped on the recurring list page (a `ChatExpenseCell`-style Cell):
  - Before: category emoji avatar
  - Subhead: "You spend" / "<Name> spends"
  - Main: "🇸🇬 SGD X.XX"
  - Description: "on <description>"
  - **After (trailing): split mode** — "Equal split" / "Split by shares" / etc.
  - No `onClick` — purely informational, lets the user verify they're
    editing the right thing without re-opening the modal.
