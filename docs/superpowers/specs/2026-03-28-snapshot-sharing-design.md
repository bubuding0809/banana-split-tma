...
The enhancement involves four main components:
...
4.  **Telegram Bot Application (`apps/bot`):** Updating legacy deep link generation to the new protocol.
...
- **`entityIdBase62`:** The 36-char UUID encoded to a Base62 string. (22 chars). _Note: Base62 only uses `[a-zA-Z0-9]`, completely eliminating any delimiter collision issues with `_`or`-`, meaning we can safely split the string by `_` during parsing without needing a "must be final segment" constraint._
...
_Example:_ `v1_g_1E2R4w_s_7N42dgm5tFLK9N8MT7fXbc`
_Total Length:_ ~37 characters. (Well within the 64-char limit).
...
- **Output:** `{ success: boolean }` _(Note: Since this is a fire-and-forget message, we do not return the `messageId` to the frontend)._
...
```markdown
📊 _Mar 26 totals_ shared by @ting
Total spent: _SGD 633.96_ (47 expenses)
...
- **0 Damage for All Users:** If _every individual user's_ damage calculates to exactly 0 (e.g., fully settled expenses included in the snapshot), the "Group Damage" section will also be omitted.
...
- If `v1_`: Split by `_`.
  ...
  - _Note on Omitted Segments:_ The parser MUST safely handle `v1` strings where the entity segments (`entityType` and `entityId`) are omitted (e.g., `v1_g_1E2R4w`), returning `undefined` for those properties.
...
- _Critical State Cleanup:_ After successfully navigating to the entity, the application state or URL params MUST be cleared so that if the user clicks "Back" to return to the chat view, they are not caught in an infinite redirect loop caused by the stale `startParams`.
...
### 4. Telegram Bot Integration (`apps/bot`)

The separate Telegram bot application in the monorepo (`apps/bot`) also generates `startapp` deep links (currently using the legacy Base64 JSON encoding) within `apps/bot/src/utils/chat.ts` via the `createChatContext` function.

To ensure the entire monorepo speaks the same URL format and strictly adheres to character limits, the bot must be updated to use the new `v1` deep linking protocol.

**Changes required:**

- Import the new `encodeV1DeepLink` utility from the shared `@packages/trpc/src/utils/deepLinkProtocol` package (or its designated export from the workspace) into the bot app.
- Update the `createChatContext` function in `apps/bot/src/utils/chat.ts` to generate `v1` payloads instead of legacy Base64 JSON strings.
- _Note on backward compatibility:_ The frontend implementation (`useStartParams.ts`) will maintain fallback parsing for legacy Base64 JSON payloads, so existing buttons rendered in past messages will continue to function correctly. However, all new messages generated by the bot will utilize the robust `v1` standard.
