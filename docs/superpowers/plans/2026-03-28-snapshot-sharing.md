...
_Critical: Implement a flag (`sessionStorage` or `useRef` if parent doesn't remount) to ensure the redirect only fires once per session, preventing infinite loops when users click back._
...
_Assuming `SnapshotPage.tsx` already reads search params and renders the list, we just need to ensure the modal state is driven by the URL or opened based on the search param, handling 404s properly._
...
<div className="flex gap-2">
  ...
  >
    {shareSnapshotMutation.isPending ? (
      ...
      <Send size={20} strokeWidth={3} style={{ color: tButtonColor }} />
    )}
  </IconButton>
