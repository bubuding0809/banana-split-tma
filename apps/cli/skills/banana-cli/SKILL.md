...
11. **Using simplified/onscreen debt to size a transfer** — `create-transfer` validates against the **direct pairwise balance** between debtor and creditor in the source chat (what `get-debts` returns), not the simplified graph (`get-simplified-debts`). Simplified balances are display-only; the simplification algorithm can reroute debt through third parties, making the direct edge A→B higher or lower than the simplified figure — e.g. the UI may show S$29.16 but the direct pairwise edge is only S$22.46, and transferring the larger simplified amount fails with a solvency error. **Always derive the transfer amount from `get-debts`; never use `get-simplified-debts` or any onscreen figure for transfer sizing.**
