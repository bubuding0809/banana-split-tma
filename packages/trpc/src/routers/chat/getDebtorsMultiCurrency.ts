...
export const getDebtorsMultiCurrencyHandler = async (
  ...
  const memberBalanceQueries =
    members
      ...
      .map(async (member) => {
        ...
        // Filter out zero balances and keep only debtor balances
        const nonZeroBalances = balances.filter((balance) =>
          isDebtor(balance.amount)
        );
  ...
  // Filter out members with no debtor balances
  const debtors = membersWithBalances.filter(
...
export default protectedProcedure
  ...
  .query(async ({ input, ctx }) => {
    return getDebtorsMultiCurrencyHandler(input, ctx.db);
  });
